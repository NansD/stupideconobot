  ...
  bot.on('custommessageReactionAdd', (messageReaction, user) => {
    if (roleActions[messageReaction.message.id]) {
      ...
      if (roleActions[messageReaction.message.id][messageReaction.emoji.name]) {
        role = Utils.guild.roles.get(
          roleActions[messageReaction.message.id][messageReaction.emoji.name].role
        );
      } else if (
        messageReaction.emoji.id &&
        roleActions[messageReaction.message.id][messageReaction.emoji.id]
      ) {
        role = Utils.guild.roles.get(
          roleActions[messageReaction.message.id][messageReaction.emoji.id].role
        );
      } else {
        return;
      }
